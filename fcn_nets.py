...
def woshialex (X, num_classes=2):
    ...
    with tf.name_scope('woshialex'):
        net = slim.conv2d(net, 8, 5, 1)
        net = slim.conv2d(net, 16, 3, 1)
        net = slim.max_pool2d(net, 2, 2)
        net = slim.conv2d(net, 32, 4, 1)
        net = slim.max_pool2d(net, 2, 2)
        net = slim.conv2d(net, 64, 4, 1)
        net = slim.max_pool2d(net, 2, 2)
        net = slim.conv2d(net, 128, 5, 1)
        net = slim.conv2d(net, 64, 5, 1)
        net = slim.conv2d_transpose(net, 32, 5, 2)
        net = slim.conv2d_transpose(net, 16, 5, 2)
        net = slim.conv2d_transpose(net, 8, 5, 2)
...
def simple (X, num_classes=2):
    ...
    with tf.name_scope('simple'):
        ...
        net = slim.conv2d_transpose(net, num_classes, 31, 16, scope='upscale', activation_fn=None)
    net = tf.identity(net, 'logits')
    return net, 16
...
def myunet (X, num_classes=2):
    ...
    with tf.name_scope('myunet'):
        ...
        net = slim.conv2d(net, 128, 4, 1)
        net = slim.max_pool2d(net, 2, 2)
                                # 1/16
        net = slim.conv2d(net, 256, 5, 1)
        net = slim.conv2d(net, 128, 5, 1)
